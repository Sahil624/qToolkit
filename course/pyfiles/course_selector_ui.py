...
try:
    ...
    def maximize_courses(*args, **kwargs): return [], 0
    def export_course_to_ipynb(*args, **kwargs): return True, ["index.ipynb"]
...
def load_metadata():
    ...
    """
    global all_courses_data, unit_structure, lo_summary_map, \
           lo_id_to_lo_object_map, cell_id_to_details_map, \
           cell_id_to_concepts_map, lo_id_to_cell_ids_map, \
           all_unique_concepts
    ...
    for root, dirs, files in os.walk(CONTENT_ROOT_DIR):
        ...
        nanomod_files = [f for f in files if f.startswith('nanomod') and f.endswith('.ipynb')]
        ...
        if len(metadata_files) == 0:
            ...
            print(f"Warning: Multiple metadata files found in {root}. Using the first one. ({metadata_file})")
        else:
            metadata_file = metadata_files[0]
        ...
        if len(nanomod_files) == 0:
            ...
            print(f"Warning: Multiple nanomod files found in {root}. Using the first one. ({nanomod_file})")
        else:
            nanomod_file = nanomod_files[0]
...
def calculate_time_for_selection(selected_lo_ids):
    ...
    def visit(cid):
        if cid in final_cell_set or cid in visiting: return
...
def create_accordion_list(structure_data, content_type='los'):
    ...
    for unit in sorted_units:
        ...
        select_all_cb = widgets.Checkbox(value=False, description="Select All in Unit", indent=False, layout=widgets.Layout(width='100%', border_bottom='1px dashed #ccc'))
        
        item_checkboxes = []
        for item in items:
            cb = widgets.Checkbox(value=False, description=item[0], indent=False)
            cb._id_value = item[1] 
...
def get_accordion_values(accordion_widget):
    selected = []
    if not accordion_widget.children: return selected
...
time_slider = widgets.IntSlider(
    value=45, min=15, max=180, step=15,
    description='Time (mins):',
    style=style, layout=widgets.Layout(width='95%')
)

activity_widget = widgets.Dropdown(
    options=[('Balanced', 'balanced'), ('Hands-on', 'experiential'), ('Theory First', 'conceptual')],
    value='balanced', description='Style:',
    style=style, layout=widgets.Layout(width='95%')
)
...
for concept in sorted(list(all_unique_concepts)):
    cb = widgets.Checkbox(value=False, description=concept, indent=False, layout=widgets.Layout(width='95%'))
...
# 6. Outputs
log_output = widgets.Output(layout=widgets.Layout(border='1px solid #ddd', height='150px', overflow_y='scroll'))
status_output = widgets.Output()


def debug_print(x):
    with log_output: print(x)
...
def on_generate_clicked(b):
    ...
    with status_output:
        ...
        selected_concepts = set([cb._id_value for cb in concept_checkboxes_list if cb.value])
        selected_target_los = get_accordion_values(target_los_accordion)

        if not selected_target_los:
            display(HTML("<h4 style='color:red'>Please select at least one Target Topic.</h4>"))
        ...
        for lo_id in all_courses_data:
            ...
            lo_concepts = [c for cid in cells for c in cell_id_to_concepts_map.get(cid, [])]
        ...
        try:
            ...
            if not selected_schedule:
                display(HTML("<h4 style='color:orange'>Unable to generate a schedule. Try increasing time or reducing targets.</h4>"))
                generate_btn.disabled = False
                generate_btn.description = 'Generate New Course'
                return
            ...
            with log_output: print(traceback.format_exc())
            generate_btn.disabled = False
            return

        scheduled_set = set(selected_schedule)
        unmet_targets = [target for target in selected_target_los if target not in scheduled_set]
...
ui = widgets.VBox([
    ...
    widgets.HBox([
        widgets.VBox([time_slider, activity_widget], layout=widgets.Layout(width='50%')),
        widgets.VBox([
            widgets.HTML("<i>Select your time constraints and preferred learning style.</i>")
    ...
    widgets.HBox([
        ...
        widgets.VBox([
            ...
            widgets.HTML("<i>Select concepts you have already mastered. Overlapping topics will be skipped automatically.</i>"),
            concepts_scroll_box
        ], layout=widgets.Layout(width='50%', padding='0 10px 0 0')),
        
        # Right Col: Targets (Accordion)
        widgets.VBox(right_cols, layout=widgets.Layout(width='50%', padding='0 0 0 10px'))
    ...
    generate_btn,
    status_output,
    widgets.Accordion(children=[log_output], titles=['Debug Logs'], selected_index=None)
])
